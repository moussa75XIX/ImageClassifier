...
TEST_ENFANT_FOLDER = os.path.join(TEST_SUBFOLDER, "enfant")
TEST_ADULTE_FOLDER = os.path.join(TEST_SUBFOLDER, "adulte")
TEST_SENIOR_FOLDER = os.path.join(TEST_SUBFOLDER, "senior")

TRAIN_ENFANT_FOLDER = os.path.join(TRAIN_SUBFOLDER, "enfant")
TRAIN_ADULTE_FOLDER = os.path.join(TRAIN_SUBFOLDER, "adulte")
TRAIN_SENIOR_FOLDER = os.path.join(TRAIN_SUBFOLDER, "senior")
...
def run():
    ...
    classe_kids_expected_outputs = [1, -1, -1]
    dataset_test_for_kids = import_dataset(TEST_ENFANT_FOLDER, classe_kids_expected_outputs)
    dataset_train_for_kids = import_dataset(TRAIN_ENFANT_FOLDER, classe_kids_expected_outputs)

    classe_adults_expected_outputs = [-1, 1, -1]
    dataset_test_for_adults = import_dataset(TEST_ADULTE_FOLDER, classe_adults_expected_outputs)
    dataset_train_for_adults = import_dataset(TRAIN_ADULTE_FOLDER, classe_adults_expected_outputs)

    classe_seniors_expected_outputs = [-1, -1, 1]
    dataset_test_for_seniors = import_dataset(TEST_SENIOR_FOLDER, classe_seniors_expected_outputs)
    dataset_train_for_seniors = import_dataset(TRAIN_SENIOR_FOLDER, classe_seniors_expected_outputs)

    final_dataset_test = dataset_test_for_kids + dataset_test_for_adults + dataset_test_for_seniors
    final_dataset_train = dataset_train_for_kids + dataset_train_for_adults + dataset_train_for_seniors
